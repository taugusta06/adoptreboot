...
class NavBar extends Component {
  constructor(props) {
    ...
    this.navBarConfig = [
      { name: "Home", link: "/" },
      { name: "Saved Pets", link: "/savedpets" },
  ...
  componentDidMount() {
    ...
    for (var navIndex in this.navBarConfig)
      if (
        this.navBarConfig[navIndex].link ===
  ...
  render() {
    return (
      <div className="navbar-container">
        <img src={dogLeft} className="doggo" alt="" />
        <div className="navbar-links">
          {this.navBarConfig.map((navBarItem, index) => {
            // console.log(navBarItem);
            return (
              <div key={index} className="nav-link-holder">
                <Link
                  className={
                    index === this.state.currentlySelectedTab
                      ? "selected"
                      : null
                  }
                  to={navBarItem.link}
        ...
        <img src={dogRight} className="doggo" alt="" />
      </div>
    );
  }
}
